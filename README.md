...
**Domain:** `Legal / Real Estate` · **Suite:** `Legal & Compliance Suite` · **Accent:** `#c084fc`
...
**Lease Analyzer** is a standalone, browser-based AI workflow app for **Legal / Real Estate**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `lease`<br>Lease | text | Yes | — | Affects input: Lease. |
...
| `tenant_landlord_role`<br>tenant/landlord role | select | No | — | Affects customization: tenant/landlord role. |
| `property_type`<br>property type | text | No | — | Affects customization: property type. |
| `risk_tolerance`<br>risk tolerance | slider | No | 50 | Affects customization: risk tolerance. |
| `term_dates`<br>term dates | text | No | — | Affects customization: term dates. |
| `fee_strictness`<br>fee strictness | slider | No | 50 | Affects customization: fee strictness. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9131
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9131`. |
...
```bash
curl http://127.0.0.1:9131/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9131/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9131 uvicorn app.main:app --host 0.0.0.0 --port 9131
```
...
- `plugins/lease-analyzer.json` for plugin metadata.
